    ...
    resources.forEach(res => {
        ...
        li.innerHTML = `
            <div style="display:flex;align-items:center;gap:7px;overflow:hidden;">
                <span class="chip-badge" style="font-size:9.5px;padding:1px 5px;${isAnim ? 'color:var(--purple);background:var(--purple-bg);border-color:var(--purple-border);' : ''}">${typeTag} ${res.id}</span>
                <span style="font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${res.name}">${res.name}</span>
            </div>
            <span style="font-size:10px;color:var(--text-muted);font-family:var(--font-mono);flex-shrink:0;">${infoText}</span>
        `;
        if (res.error) {
            li.innerHTML += `<span title="${res.error}" style="color:var(--danger);font-weight:bold;margin-left:4px;">⚠️</span>`;
        }
...
function drawPreview() {
    ...
    if (activeResource && activeResource.previewCanvas) {
        ...
        if (activeResource.type === 'animation') {
            previewInfo.innerHTML = `动画预览：${activeResource.previewFrame + 1} / ${activeResource.frameCount} 帧<br>${activeResource.fps} FPS，RGB565 大小: ${activeResource.data.length} B`;
        } else {
            previewInfo.innerHTML = `LCD坐标: (${activeResource.defaultX}, ${activeResource.defaultY})<br>RGB565 大小: ${activeResource.data.length} B`;
        }
    } else {
        previewInfo.innerHTML = `${lcdWidth} × ${lcdHeight} LCD预览，缩放 ${Math.round(previewZoom * 100)}%<br>RGB565 大小: 0 B`;
    }
}
...
function drawRulers() {
    ...
    top.strokeStyle = '#94A3B8';
    left.strokeStyle = '#94A3B8';
    top.lineWidth = 1;
    left.lineWidth = 1;
    top.font = '9px sans-serif';
    left.font = '9px sans-serif';
    ...
    for (let x = 0; x <= lcdWidth; x += logicalStep) {
        ...
        top.lineTo(displayX, x % labelStep === 0 ? 8 : 14);
        top.stroke();
    ...
    for (let y = 0; y <= lcdHeight; y += logicalStep) {
        ...
        left.lineTo(y % labelStep === 0 ? 16 : 23, displayY);
        left.stroke();
...
btnConnect.addEventListener('click', async () => {
    ...
    if (flasher.isConnected) {
        ...
        btnConnect.className = "btn-primary btn-connect-ready";
        ...
        if (success) {
            ...
            btnConnect.className = "btn-secondary btn-disconnect";
